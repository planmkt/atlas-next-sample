...
export default function Home() {
  const { count, dispatch, user, showModal, setShowModal, handleGetUser } = useHome()
    ...
    <main className={style.content}>
      ...
      <form className='flex m-3 gap-2' onSubmit={handleSubmit(handleGetUser)}>
        <input
          ...
          className="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Informe um github user" type="text" name="search" />
        <button type="submit" className={style.button}>Procurar</button>
      </form>
      {showModal &&
        <Modal.Root>
